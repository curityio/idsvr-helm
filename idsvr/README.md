...
`clusterConfJob.image.repository`| Image repository |`image.repository`
`clusterConfJob.image.tag`| Image tag |`image.tag`
`clusterConfJob.image.pullSecret`| The secret that is used to fetch images from the docker registry |`image.pullSecret`
